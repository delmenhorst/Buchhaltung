...
class OCRProcessor:
    ...
    def _extract_amount(self, text):
        ...
        amount_patterns = [
            ...
            r'(?:Gesamt|Total|Summe|Betrag|Amount|Brutto).*?(\d{1,3}(?:\.\d{3})*,\d{2})\s*€',
            ...
            r'(?:Gesamt|Total|Summe|Betrag|Amount|Brutto).*?(\d+,\d{2})\s*€',
            r'€\s*(\d+,\d{2})',
            r'(\d+,\d{2})\s*€',
        ]
    ...
    def _predict_category(self, text):
        ...
        categories = {
            'Büro': [
                ...
                'kabel', 'usb', 'festplatte', 'maus', 'tastatur', 'bürobedarf',
                'hosting', 'domain', 'server', 'cloud'
            ],
            'Raum': [
                'miete', 'rent', 'studio', 'atelier', 'büro', 'workspace', 'coworking',
